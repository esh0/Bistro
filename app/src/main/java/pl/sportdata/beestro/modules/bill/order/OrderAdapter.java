...
import pl.sportdata.beestro.utils.RomanNumberUtils;
...
class OrderAdapter extends AbstractExpandableItemAdapter<OrderAdapter.BillGroupViewHolder, OrderAdapter.OrderBillEntryViewHolder>
    ...
    public void onBindChildViewHolder(OrderBillEntryViewHolder holder, int groupPosition, int childPosition, int viewType) {
        ...
        ViewUtils.addTouchDelegate(holder.dragHandle);

        holder.container.setOnClickListener(mSwipeableViewContainerOnClickListener);

        Entry entry = childItem.getEntry();
        int id = entry.getItemId();
        BeestroObject item = beestroDataProvider.getItem(id);
        if (item == null) {
            item = beestroDataProvider.getDiscount(id);
        }
        if (item == null) {
            item = beestroDataProvider.getPaymentType(id);
        }
        if (item == null) {
            item = beestroDataProvider.getMarkup(id);
        }
        ...
        if (entry.isNew() || entry.isModified()) {
            ...
            @ColorInt int valueColor = ContextCompat.getColor(context, groupItem.isActive() ? R.color.colorAccentDark : R.color.gray_a8);
