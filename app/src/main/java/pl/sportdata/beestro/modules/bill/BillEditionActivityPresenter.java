...
import pl.sportdata.beestro.modules.bills.split.SplitActivity;
import pl.sportdata.beestro.modules.bills.split.SplitActivityImpl;
...
public class BillEditionActivityPresenter extends BasePresenter<BillEditionActivity> implements BillEditionPagerAdapter.Listener {
    ...
    private void setBillLabels() {
        ...
        getActivity().showExtraDescriptionText(bill.getExtraDescription());
    }
    ...
    public void onTableButtonClicked() {
        String tableGuestNumber = String.valueOf(bill.getTableNumber());
        if (bill.getGuestNumber() != 0) {
            tableGuestNumber += "." + String.valueOf(bill.getGuestNumber());
        }

        getActivity().showEditTableNumber(tableGuestNumber);
    }
    ...
    public void onTableNumberChanged(String number) {
        boolean valid = !TextUtils.isEmpty(number);
        if (valid) {
            int tableId = BillUtils.getBillTableId(number);
            int guestNumber = BillUtils.getBillGuestNumber(number);
        ...
        if (TextUtils.isEmpty(error)) {
            ...
            if (bill != null) {
                ...
                new AlertDialog.Builder(getActivity()).setTitle(R.string.message_from_gh).setMessage(
                        String.format(getActivity().getString(R.string.bill_closed), String.valueOf(BillUtils.getBillDeviceId(originalBill)),
                                String.valueOf(BillUtils.getBillId(originalBill)))).setPositiveButton(R.string.close, new DialogInterface.OnClickListener() {
                    @Override
    ...
    public void onExtraDescriptionEdited(@Nullable String description) {
        bill.setExtraDescription(description);
        bill.setModified(true);
        getActivity().showExtraDescriptionText(description);
    }

    public void onEditDescriptionClicked() {
        getActivity().showExtraDescriptionDialog();
    }
}
